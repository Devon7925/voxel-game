...
pub(crate) struct ServerState {
    ...
    lobbies: HashMap<RoomId, (HashSet<PeerId>, Lobby)>,
}
    ...
    fn remove_peer(&mut self, peer_id: &PeerId) -> Option<Peer> {
        ...
        if let Some(ref peer) = peer {
            ...
            _ = self
                .lobbies
                .get_mut(&peer.room)
                .map(|room| room.0.remove(peer_id));
        }
        peer
    }
...
pub(crate) async fn ws_handler(
    ...
    let room_id = room_id.map(|path| path.0).unwrap();

    // Finalize the upgrade process by returning upgrade callback to client
    ws.on_upgrade(move |websocket| handle_ws(websocket, state, room_id))
}

pub(crate) async fn lobby_lister(
    State(state): State<Arc<Mutex<ServerState>>>,
) -> impl IntoResponse {
...
pub(crate) async fn lobby_creator(
    State(state): State<Arc<Mutex<ServerState>>>,
...
async fn handle_ws(websocket: WebSocket, state: Arc<Mutex<ServerState>>, requested_room: RoomId) {
...

    {
        ...
        let Ok(peers) = add_peer_state.add_peer(Peer {
            uuid: peer_uuid,
            sender: sender.clone(),
